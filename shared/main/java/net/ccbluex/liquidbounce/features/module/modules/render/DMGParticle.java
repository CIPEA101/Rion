...
public class DMGParticle extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (floatValue != health) {
            ...
            if (floatValue - health < 0.0f) {
                text = "§d" + roundToPlace((floatValue - health) * -1.0f, 1);
            } else {
                text = "§b" + roundToPlace(floatValue - health, 1);
            }
    ...
    public void onRender(Render3DEvent event) {
        for (Particles p : this.particles) {
            ...
            mc.getFontRendererObj().drawString(p.text, (float)(-( mc.getFontRendererObj().getStringWidth(p.text) / 2)), (float)(-( mc.getFontRendererObj().getFontHeight() - 1)), 0, true);
