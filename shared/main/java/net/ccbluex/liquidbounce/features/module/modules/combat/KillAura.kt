...
class KillAura : Module() {
    ...
    // CPS - Attack speed
    private val maxCPS: IntegerValue = object : IntegerValue("MaxCPS", 18, 1, 40) {
    ...
    private val minCPS: IntegerValue = object : IntegerValue("MinCPS", 12, 1, 40) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val i = maxCPS.get()
            if (i < newValue) set(i)
    ...
    val hurtTimeValue = IntegerValue("HurtTime", 10, 0, 30)
    ...
    // Turn Speed
    private val maxTurnSpeed: FloatValue = object : FloatValue("MaxTurnSpeed", 160f, 0f, 360f) {
    ...
    private val minTurnSpeed: FloatValue = object : FloatValue("MinTurnSpeed", 80f, 0f, 360f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
            val v = maxTurnSpeed.get()
            if (v < newValue) set(v)
        }
    }
    private val hitableValue2 = BoolValue("RionHitable",true)
    ...
    private fun updateHitable() {
        ...
        if (hitableValue2.get()) {
            if(minTurnSpeed.get() >= 40F)
            hitable = true
            return
        }
