...
import net.ccbluex.liquidbounce.features.module.modules.tomk.CancelC03
...
@ModuleInfo(name = "Velocity", description = "修复版", category = ModuleCategory.COMBAT)
class Velocity : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf(
            "HytBest","HuaYuTingJump","Custom","AAC4","Simple", "SimpleFix",
            "AAC", "AACPush", "AACZero",
            "Reverse", "SmoothReverse", "Jump", "AAC5Reduce",
            "HytPacketA" ,"Glitch","HytCancel","HytTick","Vanilla",
            "HytTest","HytNewTest","HytPacket","NewAAC4","Hyt","FeiLe","HytMotion","S27",
            "NewHytMotion","HytPacketB","HytMotionB","HytPacketFix","NoXZ","NoXZfix",
            "Rion","RionPit","Rion4v4","TEST","HytPlus",
            "GrimTest","grim-packet","grim-motion","Grim2","GrimFull"), "Vanilla")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when (modeValue.get().toLowerCase()) {
            "hytplus" ->  if (thePlayer.hurtTime > 0) {
                mc.thePlayer!!.motionY = -LiquidBounce.combatManager.target!!.motionY
                mc.thePlayer!!.motionX = -LiquidBounce.combatManager.target!!.motionX
                mc.thePlayer!!.motionZ = -LiquidBounce.combatManager.target!!.motionZ
            }
            "hytbest"->{
            ...
            "grim-motion" -> {
                if (mc.thePlayer!!.hurtTime > 0) {
                    mc2.player.addVelocity(-1.0E-7,-1.0E-7,-1.0E-7)
                    mc2.player.addVelocity(-1.0E-7,-1.0E-7,-1.0E-7)
                    mc2.player.addVelocity(-1.0E-7,-1.0E-7,-1.0E-7)
                }
            }
            ...
            "rion" -> {
                if (thePlayer.hurtTime > 0) {
                    thePlayer.motionX = thePlayer.motionX - 1.0E-7
                    thePlayer.motionY = thePlayer.motionY - 1.0E-7
                    thePlayer.motionZ = thePlayer.motionZ - 1.0E-7
                    thePlayer.isAirBorne = true
                }
                if (thePlayer.hurtTime > 0 && !thePlayer.onGround) {
                    thePlayer.motionX = thePlayer.motionX * 0.0F
                    thePlayer.motionZ = thePlayer.motionZ * 0.0F
                }
                if (thePlayer.hurtTime > 9) {
                    thePlayer.motionX = thePlayer.motionX - 1.1E-7
                    thePlayer.motionY = thePlayer.motionY - 1.1E-7
                    thePlayer.motionZ = thePlayer.motionZ - 1.2E-7
                    canCleanJump = true
                    thePlayer.isAirBorne = true
                }
                if (thePlayer.hurtTime > 2 && !thePlayer.isDead && mc.thePlayer!!.hurtTime <= 5) {
                    thePlayer.motionX = thePlayer.motionX * 0.35
                    thePlayer.motionZ = thePlayer.motionZ * 0.35
                    thePlayer.motionY = thePlayer.motionY / 1.781145f
                    return
                }
                return
            ...
            "hyt" -> {
                if (thePlayer.hurtTime > 0 && !thePlayer.onGround) {
                    thePlayer.motionX *= RenderUtils.drawJelloShadow()
                    thePlayer.motionZ *= RenderUtils.drawImage4()
                }
            }
        ...
        val packet = event.packet
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "rion4v4" -> {
                    if (mc.thePlayer != null) {
                        if (classProvider.isSPacketEntityVelocity(event.packet)) {
                            ...
                            if (attack) {
                                if (classProvider.isCPacketConfirmTransaction(event.packet)) {
                ...
                "grim" -> {
                    ...
                    var cancelC03 = LiquidBounce.moduleManager.getModule(CancelC03::class.java) as CancelC03
                    cancelC03.info = true
                    //velocityInput = true
                    packetEntityVelocity.motionX = (packetEntityVelocity.motionX * 0).toInt()
                    packetEntityVelocity.motionY = (packetEntityVelocity.motionY * 0).toInt()
                    packetEntityVelocity.motionZ = (packetEntityVelocity.motionZ * 0).toInt()
                }
                "cancel" -> {
                    event.cancelEvent()
                }
                "grim-packet" ->{
                    mc.thePlayer!!.motionX += -0.01001400514
                    mc.thePlayer!!.motionZ += 0.0001011101
                    mc.thePlayer!!.motionY += -0.0010101010
                }
