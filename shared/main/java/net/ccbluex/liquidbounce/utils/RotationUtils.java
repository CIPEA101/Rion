...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static Rotation getRotationsEntity(IEntityLivingBase entity) {
        return RotationUtils.getRotations(entity.getPosX(), entity.getPosY() + entity.getEyeHeight() - 0.4, entity.getPosZ());
    }
    ...
    public static VecRotation searchBestPositionAndRotation(IAxisAlignedBB bb, boolean outborder, boolean random, boolean predict, boolean throughWalls, float distance) {
        if (outborder) {
        ...
        WVec3 eyes = mc.getThePlayer().getPositionEyes(1F);
     ...
     * @return center
     */
    public static VecRotation searchCenter1(final IAxisAlignedBB bb, final boolean outborder, final boolean random,
                                            final boolean predict, final boolean throughWalls, final float distance, boolean xS, boolean yS) {
        if (outborder) {
            final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * (x * 0.3 + 1.0), bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * (y * 0.3 + 1.0), bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * (z * 0.3 + 1.0));
            return new VecRotation(vec3, toRotation(vec3, predict));
        }
        ...
        for(double xSearch = 0.15D; xSearch < 0.85D; xSearch += 0.1D) {
            for (double ySearch = 0.15D; ySearch < 1D; ySearch += 0.1D) {
                for (double zSearch = 0.15D; zSearch < 0.85D; zSearch += 0.1D) {
                    final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * (xS ? xSearch : 0.5),
                            bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * (yS ? ySearch : 0.5), bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * (xS ? zSearch : 0.5));
                    final Rotation rotation = toRotation(vec3, predict);
                    final double vecDist = eyes.distanceTo(vec3);
