...
import tomk.render.BlurBuffer
import tomk.render.Palette
...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    companion object {
        ...
        fun defaultClient(): Text {
            ...
            text.displayString.set( "Rion | Fps:%fps% | %serverip%")
    ...
    private val colorModeValue = ListValue("Text-Color", arrayOf("Custom","Fade", "Astolfo", "NewRainbow","Gident"), "Custom")
    ...
    private val blurValue = BoolValue("Top-Blur", true)
    private val shadowValue = ListValue("Top-Shadow", arrayOf("None", "Basic", "Thick"), "Thick")
    private val radiusValue = FloatValue("Top-Radius", 4.25f, 0f, 10f)
    ...
    private var fontValue = FontValue("Font", Fonts.
    sfbold40)
...
//            "worldTime" -> return SessionUtils.getFormatWorldTime()
    ...
    override fun drawElement(): Border? {
        ...
        val mwc1 = Color(VisualColor.r.get(),VisualColor.b.get(),VisualColor.g.get()).rgb
        val mwc2 = Color(VisualColor.r2.get(),VisualColor.b2.get(),VisualColor.g2.get()).rgb
        if (this.Mode.get().equals("Top")) {
            when (shadowValue.get()) {
                "Basic" ->   RenderUtils.drawShadow(-2.5f, -2.5f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat())
                "Thick" -> {
                    RenderUtils.drawShadow(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat())
                    RenderUtils.drawShadow(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat())
                }
            }
            if (blurValue.get()){
                ...
                GL11.glPushMatrix()
                BlurBuffer.blurRoundArea(renderX.toFloat(), renderY.toFloat(), (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat()
                        , radiusValue.get().toInt())
                GL11.glPopMatrix()
                GL11.glTranslated(renderX, renderY, 0.0)
            }
            RoundedUtil.drawRound(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat(), 0F, Color(0,0,0,100))
            RenderUtils.drawGradientSideways(-2.0,-3.0, (fontRenderer.getStringWidth(displayText) + 2).toDouble(), -2.0, mwc1, mwc2)
        }
            ...
            if (Mode.get() == "Skeet") {
                ...
                for (i in 0..(amountValue.get()-1)) {
                    ...
                    RenderUtils.drawGradientSideways(-1.4 + barStart, -2.7, -1.4 + barEnd, -2.0,
                            when {
                                rainbow -> 0
                                colorMode.equals("Fade", ignoreCase = true) -> Palette.fade2(Color(VisualColor.r.get(),VisualColor.b.get(),VisualColor.g.get()), i * distanceValue.get(), displayText.length * 200).rgb
                                ...
                                colorMode.equals("Gident", ignoreCase = true) -> RenderUtils.getGradientOffset(Color(VisualColor.r.get(),VisualColor.b.get(),VisualColor.g.get()), Color(VisualColor.r2.get(), VisualColor.b2.get(), VisualColor.g2.get(),1), (Math.abs(System.currentTimeMillis() / gidentspeed.get().toDouble() + i * distanceValue.get()) / 10)).rgb
        ...
        if(char.get()){
            ...
            RainbowFontShader.begin(rainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                fontRenderer.drawString(displayText, 0F, 0F, when {
                    rainbow -> 0
                    colorMode.equals("Fade", ignoreCase = true) -> Palette.fade2(Color(VisualColor.r.get(),VisualColor.b.get(),VisualColor.g.get()), counter[0] * 100, displayText.length * 200).rgb
                    ...
                    colorMode.equals("Gident", ignoreCase = true) -> RenderUtils.getGradientOffset(Color(VisualColor.r.get(),VisualColor.b.get(),VisualColor.g.get()), Color(VisualColor.r2.get(), VisualColor.b2.get(), VisualColor.g2.get(), 1), (Math.abs(System.currentTimeMillis() / gidentspeed.get().toDouble() + counter[0]) / 10)).rgb
                    else -> color
                }, shadow.get())
    ...
    fun setColor(c: Color): Text {
        VisualColor.r.set(c.red)
        VisualColor.b.set(c.green)
        VisualColor.g.set(c.blue)
        return this
    }
