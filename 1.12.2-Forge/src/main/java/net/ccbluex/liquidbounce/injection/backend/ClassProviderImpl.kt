...
object ClassProviderImpl : IClassProvider {
    ...
    override fun createEntityLightningBolt(world: IWorld, posX: Double, posY: Double, posZ: Double, effectOnly: Boolean):IEntity = EntityLightningBolt(world.unwrap(), posX, posY ,posZ, effectOnly).wrap()
    ...
    override fun isCPacketTryUseItem(obj: Any?): Boolean = obj is PacketImpl<*> && obj.wrapped is CPacketPlayerTryUseItem
    ...
    override fun getPotionEnum(type: PotionType): IPotion {
        return PotionImpl(when (type) {
            ...
            PotionType.GLOWING -> MobEffects.GLOWING
            PotionType.JUMP -> MobEffects.JUMP_BOOST
        })
    }
